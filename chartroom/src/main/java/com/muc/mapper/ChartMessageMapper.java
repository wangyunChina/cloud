...
public interface ChartMessageMapper {
    ...
    @Select({
            ...
            "id, message_from, message_to, send_time, message, isread",
            "from chart_message",
            "where message_from = #{from,jdbcType=VARCHAR}",
            "and message_to=#{to,jdbcType=VARCHAR} and isread=false"
    })
    @Results({
            @Result(column="id", property="id", jdbcType=JdbcType.INTEGER, id=true),
            @Result(column="message_from", property="messageFrom", jdbcType=JdbcType.VARCHAR),
    ...
    ArrayList<ChartMessage> selectByProporty(String from, String to);
    @Select({
            "select",
            "id, message_from, message_to, send_time, message, isread",
            "from chart_message",
            "where message_to = #{to,jdbcType=VARCHAR}",
            "and isread=false group by message_from"
    })
    @Results({
            @Result(column="id", property="id", jdbcType=JdbcType.INTEGER, id=true),
            @Result(column="message_from", property="messageFrom", jdbcType=JdbcType.VARCHAR),
    ...
    ArrayList<ChartMessage> selectNotReadMan(String to);
